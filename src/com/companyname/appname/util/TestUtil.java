...
public class TestUtil {
	...
	public static Object[][] getData(String testCase,Xls_Reader xls) {
		...
		while(!xls.getCellData("Sheet1",0,testStartRowNum).equals(testCase)){ // LoginTest
			testStartRowNum++;
		}
		...
		int rows=0;
		while(!xls.getCellData("Sheet1",0,dataStartRowNum+rows).equals("")){
			rows++;
		...
		int cols=0;
		while(!xls.getCellData("Sheet1", cols,colStartRowNum).equals("")){
			cols++;
		}
		...
		for(int rNum=dataStartRowNum;rNum<dataStartRowNum+rows;rNum++ ){
			...
			String key=xls.getCellData("Sheet1",cNum ,colStartRowNum);	
			String value=xls.getCellData("Sheet1",cNum, rNum);
